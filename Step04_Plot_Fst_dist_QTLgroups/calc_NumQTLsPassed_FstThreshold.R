...
#fst_thr_600K <- median(Fst_600K$Fst)+0.25*(median(Fst_600K$Fst))
...
## Prioritize the top 12 SNPs surrounding qualified QTLs based on Fst scores
#Top12SNPsFst <-  Passed_QTLs %>% dplyr::group_by(Qid) %>%
 #                 dplyr::slice_max(order_by = Fst, n = 12, with_ties = FALSE) %>%
  #                dplyr::mutate(dplyr::across(where(is.numeric), ~ round(., 6))) %>%
   #               dplyr::arrange(order(gtools::mixedorder(Mid))) %>%
    #              dplyr::arrange(order(gtools::mixedorder(Qid)))
#head(Top12SNPsFst)

## determine the index of top SNPs selected based on Fst near each qualified  QTL
#Top12SNPsFst <- Top12SNPsFst %>% dplyr::mutate(Mid = sub("M", "", Mid)) %>%
 #                                 ungroup() %>% dplyr::select(Mid) %>%
 #                                 dplyr::distinct(Mid, .keep_all=TRUE)

#write.table(Top12SNPsFst,paste0("Top12SNPsFst_", SNPwindows),col.names=F, row.names=F,quote=F)

...
###########################################################
###   Define the windows of 200 SNPs around 2000 QTLs  ####
###########################################################
SNPwindows<-"200win"
SNPmargin<- 100

## function to extract 100 SNPs before and after each QTL index 
...
## Prioritize random 3 SNPs surrounding qualified QTLs based on Fst scores
set.seed(123)
Random3SNPsFst <- Passed_QTLs %>% dplyr::group_by(Qid) %>%
...
readr::write_csv(Random3SNPsFst, paste0(SNPwindows,"_","random3snpselection.csv"))

## determine the index of randomly selected SNPs based on Fst scores for qualified QTLs
Random3SNPsFst <- Random3SNPsFst %>% dplyr::mutate(Mid = sub("M", "", Mid)) %>%
                                       ungroup() %>% dplyr::select(Mid) %>%
                                     dplyr::distinct(Mid, .keep_all=TRUE)

write.table(Random3SNPsFst,paste0("Rnd3SNPsFst_", SNPwindows),col.names=F, row.names=F,quote=F)

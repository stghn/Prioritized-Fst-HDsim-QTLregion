...
setwd("C:/Users/Sajjad.Toghiani/Downloads/WQP_h2_rep1/WQP_h2_rep1/h2010/W1Q1P2_h2010_rep1")
...
summary(QTL$addQtl.percent)
length(QTL$addQtl.percent)

# Calculate mean and SD for the top 95th percentile group
top95_stats <- QTL %>%
         dplyr::filter(addQtl.percent >= quantile(addQtl.percent, probs = 0.95)) %>%
         dplyr::summarise(mean_GV = mean(addQtl.percent, na.rm = TRUE), 
                          sd_GV = sd(addQtl.percent, na.rm = TRUE),
                          n_selected = n() )

# Calculate mean and SD for the middle 25th-75th percentile group
middle2575_stats <- QTL %>%
                  dplyr::filter(addQtl.percent >= quantile(addQtl.percent, probs = 0.25) &  
                          addQtl.percent <= quantile(addQtl.percent, probs = 0.75)) %>%
                  dplyr::summarise(mean_GV = mean(addQtl.percent, na.rm = TRUE), 
                                   sd_GV = sd(addQtl.percent, na.rm = TRUE),
                                   n_selected = n() )
...
bottom5_stats <- QTL %>%
  		dplyr::filter(addQtl.percent <= quantile(addQtl.percent, probs = 0.05)) %>%
  		dplyr::summarise(mean_GV = mean(addQtl.percent, na.rm = TRUE), 
                                 sd_GV = sd(addQtl.percent, na.rm = TRUE),
                                 n_selected = n() )
...
sample2575Q_stats <-  QTL %>%
          ...
          # Calculate mean and standard deviation of GV for the selected 25 QTLs %>%
          dplyr::summarise(mean_GV = mean(addQtl.percent, na.rm = TRUE), 
                           sd_GV = sd(addQtl.percent, na.rm = TRUE),
                           n_selected = n() )
...
readr::write_csv(combined_stats, "GV_stats_QTLgroups_W1Q1P2.csv")


# Extract QTLs where addQtl.percent is in the top 5% (>= 95th percentile)
top95qtl <- QTL %>%
         dplyr::filter(addQtl.percent >= quantile(addQtl.percent, probs = 0.95)) %>%  # Filter top 5%
...
sample2575qtl <-  QTL %>%
          dplyr::filter(addQtl.percent >= quantile(addQtl.percent, probs = 0.25) &  
                 addQtl.percent <= quantile(addQtl.percent, probs = 0.75)) %>%  # Filter within  25th-75th percentile
          dplyr::sample_n(size = 25, replace = FALSE) %>%  # Randomly select 25 QTLs
          dplyr::arrange(chr) %>%  #Arrange by chromosome
          dplyr::pull(Index) %>%  #Extract the Index column
          gtools::mixedsort() # Sorting alphanumeric vector
...
# Extract QTLs where addQtl.percent is in the bottom 5% (<= 5th percentile)
bottom5qtl <- QTL %>%
            filter(addQtl.percent <= quantile(addQtl.percent, probs = 0.05)) %>%  #Filter bottom 5%
            dplyr::arrange(chr) %>% # Arrange by chromosome
            dplyr::pull(Index) %>%  # Extract the Index column
            gtools::mixedsort() # Sorting alphanumeric vector
...
readr::write_csv(combined_SNPwin_stats, "Fst_stats_50SNPwin_GV_QTLgroups_W1Q1P2.csv")

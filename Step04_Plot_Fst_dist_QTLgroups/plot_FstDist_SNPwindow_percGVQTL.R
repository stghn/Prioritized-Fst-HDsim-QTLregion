...
Fst_SNPwin_top2QTL <- SNPwindows_top2QTL %>% dplyr::left_join(Fst_600K, by = "Mid") %>%
                                             ...
                                             dplyr::mutate(Chr = factor(Chr, levels = c("chr12","chr16")))
...
Fst_SNPwin_middle2QTL <- SNPwindows_middle2QTL %>% dplyr::left_join(Fst_600K, by = "Mid") %>% 
						   dplyr::mutate(Chr = paste("chr", chr, sep = "")) %>%
                                                   dplyr::mutate_at(c('Chr','Qid'), as.factor) %>%
                                                   dplyr::mutate(Chr = factor(Chr, levels = c("chr3","chr21")))
...
mod_data <- Fst_SNPwin_top2QTL %>%
              ...
              dplyr::mutate(Top_QTL = factor(Top_QTL, labels = c("QTL240", "QTL313"))) %>%
...
topplot <- ggplot(mod_data, aes(x = Mid, y = Fst, fill = Top_QTL)) +  
  ...
  facet_wrap(~ ifelse(Chr == "chr12", "QTL240", ifelse(Chr == "chr16", "QTL313", Chr)), 
             ncol = 2, scales = "free_x",labeller = as_labeller(c("QTL240" = "QTL240; Large QTL variance",
                         "QTL313" = "QTL313; Large QTL variance"))) +
  ...
  labs(y = "Fst score", fill = "Large QTL variance") +
...
#mod_data <- Fst_SNPwin_middle2QTL %>% dplyr::rename(Middle_QTL=Qid) %>%
 #                                    dplyr::mutate_at(c('Middle_QTL'), as.factor) %>%
  #                                   dplyr::group_by(Middle_QTL) %>%  dplyr::mutate(Fst = ifelse(Fst < 0, 0, Fst)) 
                                 
mod_data <- Fst_SNPwin_middle2QTL %>%
              dplyr::rename(Middle_QTL = Qid) %>%
              dplyr::mutate(Middle_QTL = factor(Middle_QTL, labels = c("QTL53", "QTL380"))) %>%
...
middleplot <- ggplot(mod_data, aes(x = Mid, y = Fst, fill = Middle_QTL)) +
  geom_bar(stat = "identity", width = 0.4, position = position_dodge(width = 0.75)) +
  facet_wrap(~ ifelse(Chr == "chr3", "QTL53", ifelse(Chr == "chr21", "QTL380", Chr)),
             ncol = 2, scales = "free_x",labeller = as_labeller(c("QTL53" = "QTL53; Medium QTL variance",
                         "QTL380" = "QTL380; Medium QTL variance"))) +
  scale_fill_manual(values = c("green4", "blue4")) +
  ...
  labs(y = "Fst score", fill = "Medium QTL variance") +
  scale_y_continuous(limits = c(0, 0.004)) +
...
bottomplot <- ggplot(mod_data, aes(x = Mid, y = Fst, fill = Bottom_QTL)) +
  geom_bar(stat = "identity", width = 0.4, position = position_dodge(width = 0.75)) +
  facet_wrap(~Chr, ncol = 2, scales = "free_x",labeller = as_labeller(c("QTL85" = "QTL85; Small QTL variance",
                         "QTL436" = "QTL436; Small QTL variance")))  +
  ...
  labs(y = "Fst score", fill = "Small QTL variance") +
  scale_y_continuous(limits = c(0, 0.004)) +
  # Ensure the legend has thicker edges
...
ggplot2::ggsave(paste0(SNPwindows,"_","around2QTLs_percGVQTL.png"), unit="in",width = 20, height = 20, dpi = 300)
